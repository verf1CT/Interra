...
> [!IMPORTANT]
> **Настройка Telegram-бота и прокси в РФ:**
> 1. **Запросите токен бота**: Админ должен **запросить у разработчика** рабочий `TELEGRAM_BOT_TOKEN` и `TELEGRAM_CHAT_ID` (или прописать свой из `@BotFather`).
> 2. **Настройте прокси**: Так как сервер расположен на территории РФ, для работы Telegram-бота **необходимо прописать зарубежный прокси** в файле `/opt/interra/server/.env`:
>    `TELEGRAM_PROXY=http://vps-frankfurt.duckdns.org:8080` (или `socks5://...`).
