...
export class BroadcastService {
    deviceRepo;
    broadcastRepo;
    constructor(deviceRepo = new DeviceRepository(), broadcastRepo = new BroadcastRepository()) {
    ...
    async runBroadcast(params) {
        ...
        if (link)
            fcmData.link = link;
        logger.info({ broadcastId, recipients: tokens.length, target }, '[broadcast] Отправка рассылки...');
        ...
        logger.info({ broadcastId, successCount, failureCount }, '[broadcast] Рассылка завершена');
